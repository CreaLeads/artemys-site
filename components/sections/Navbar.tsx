    ...
    <header className="fixed inset-x-0 top-3 z-50 px-4 sm:top-5">
      <nav
        className={`glass-blur glass-sheen mx-auto flex max-w-6xl items-center justify-between gap-4 rounded-full py-2.5 pl-5 pr-2.5 transition-all duration-300 ${
          scrolled ? "bg-glass/[0.1] shadow-2xl" : ""
        }`}
      >
        <a href="/" className="relative z-10 text-cream" aria-label="Artémys — accueil">
          <Logo />
        </a>
        ...
        <ul className="hidden items-center gap-1 lg:flex">
          {NAV_LINKS.map((link) => (
            <li key={link.href}>
              <a
                href={link.href}
                className="rounded-full px-4 py-2 text-sm font-medium text-cream/75 transition-colors hover:bg-glass/10 hover:text-cream"
              >
                {link.label}
              </a>
            </li>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            aria-expanded={open}
            onClick={() => setOpen((v) => !v)}
            className="flex h-11 w-11 items-center justify-center rounded-full border border-glass/15 bg-glass/[0.07] text-cream lg:hidden"
          >
            ...
            {NAV_LINKS.map((link) => (
              <li key={link.href}>
                <a
                  ...
                  className="block rounded-2xl px-4 py-3 text-base font-medium text-cream/85 transition-colors hover:bg-glass/10"
                >
                  {link.label}
                </a>
              </li>
