...
export function Prestations() {
  ...
  // Synchronise l'onglet quand on clique sur une carte de pôle.
  useEffect(() => {
    function onSelect(e: Event) {
      const id = (e as CustomEvent<string>).detail;
      if (PRESTATIONS.some((c) => c.id === id)) setActive(id);
    }
      ...
      <div className="container-custom">
        <Reveal className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="heading-lg mt-5">
            Besoin d'un <span className="text-orange">support précis ?</span>
          </h2>
              ...
              <button
                ...
                className={`shrink-0 rounded-full px-5 py-2.5 text-sm font-semibold transition-all ${
                  active === cat.id
                    ...
                    : "border border-white/15 bg-white/[0.05] text-cream/70 hover:bg-white/10"
                }`}
              >
                {cat.pole}
              </button>
        ...
        <div className="glass glass-sheen mt-8 rounded-[2rem] p-7 sm:p-10">
          <div className="mb-7 flex flex-wrap items-end justify-between gap-3 border-b border-white/10 pb-6">
            <div>
          ...
          <div className="grid gap-x-10 gap-y-5 sm:grid-cols-2">
            {current.items.map((item) => (
              <div
                key={item.name}
                className="flex flex-col gap-1 border-b border-white/[0.06] pb-5"
              >
